...
describe('hike schedule rules', () => {
  it('offers the three supported hike windows and marks the recommended times', () => {
    ...
    expect(HIKE_TIME_OPTIONS.night.some((option) => option.time === '02:00 PM' && option.recommended)).toBe(true);
    expect(HIKE_TIME_OPTIONS.night.at(-1)?.time).toBe('04:00 PM');
  ...
  it('rejects times outside the selected hike window', () => {
    ...
    expect(isValidHikeTime('night', '02:00 PM')).toBe(true);
    expect(isValidHikeTime('night', '04:00 PM')).toBe(true);
    expect(isValidHikeTime('night', '05:00 PM')).toBe(false);
    expect(isValidHikeTime('night', '08:00 PM')).toBe(false);
    expect(isValidHikeTime('overnight', '02:00 PM')).toBe(true);
    expect(isValidHikeTime('overnight', '03:00 PM')).toBe(true);
    expect(isValidHikeTime('overnight', '04:00 PM')).toBe(true);
    expect(isValidHikeTime('overnight', '05:00 PM')).toBe(false);
    expect(isValidHikeTime('overnight', '10:00 PM')).toBe(false);
  });
