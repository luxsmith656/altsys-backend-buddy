    ...
    <main className="min-h-screen px-3 pb-24 pt-20 sm:px-4 sm:pt-24">
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        {view === 'map' ? (
          ...
          <div className="grid gap-4 lg:grid-cols-2">{visibleRecords.map((record) => <Card key={record.bookingId} className="glass-card"><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-base">{record.leadName || 'Unnamed team'}</CardTitle><p className="mt-1 text-xs text-muted-foreground">{record.locationName} · {record.bookingDate}</p></div><Badge variant="outline">{record.groupSize} people</Badge></div><p className="text-sm text-muted-foreground">Contact: <span className="text-foreground">{record.contactNumber || 'Not provided'}</span></p></CardHeader><CardContent className="space-y-3"><div className="space-y-2">{getPeople(record).map((person, index) => <div key={`${record.bookingId}-${person.name}-${index}`} className="rounded-md border border-border/40 bg-background/30 p-3"><div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm font-medium"><span>{person.name || 'Unnamed person'}</span>{person.age != null && <span className="text-muted-foreground">Age {person.age}</span>}{person.sex && <span className="text-muted-foreground">{person.sex}</span>}</div>{person.medicalNotes && <p className="mt-1 text-sm font-medium text-destructive">Medical note: {person.medicalNotes}</p>}</div>)}</div></CardContent></Card>)}</div>
        )}
      </div>
    </main>
  );
